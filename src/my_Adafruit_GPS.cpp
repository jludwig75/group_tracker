...
boolean Adafruit_GPS::parse(char *nmea, boolean *date_and_time_update) {
  // do checksum check
  bool time_updated = false;
  *date_and_time_update = false;
  ...
  if (strstr(nmea, "$GPRMC")) {
   // found RMC
    ...
    p = strchr(p, ',')+1;
    float timef = atof(p);
    uint32_t time = round(timef);
    hour = time / 10000;
    minute = (time % 10000) / 100;
    seconds = (time % 100);
    // Serial.println("GPS time set");
    // Serial.print("timef value is "); Serial.println(timef);
    // Serial.print("time value is "); Serial.println(time);
    ...
    if (',' != *p)
    {
      ...
      year = (fulldate % 100);
      // Serial.println("GPS date set");
      // Serial.println(nmea);
      if (time_updated)
      {
        *date_and_time_update = true;
      }
    }
